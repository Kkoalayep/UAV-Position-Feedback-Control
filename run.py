...
# There are 3 function added to the code, which are 'reward_cal'(calculate the reward from the current position with
# the target position), 'write_results' (write the reward to the file) and 'wrap_angle'. Another time setting is added
# so that the simulation only runs the time of 'max_allow' during training.
max_allow = 5
...
def reward_cal(reward, current, target):
    # Error calculation
    current_position = np.array(current[:3])
    pos_error = np.abs(current_position - np.array(target[:3]))
    yaw_error = abs(wrap_angle(current[5] - target[3]))

    # Combine the error to an array
    error = np.hstack((pos_error, yaw_error))

    # Reward design
    for i in range(4):
        error[i] = error[i] / abs(target[i])
        if error[i] >= 0.03:
            reward[i] -= np.exp(error[i]) * 10 * real_time * real_time
        reward[i] += np.exp(-error[i]) * 20

    return reward

def write_results(reward):
    with open("result.txt", "w") as f:
        f.write(f"reward_x={reward[0]:.4f}\n")
        f.write(f"reward_y={reward[1]:.4f}\n")
        f.write(f"reward_z={reward[2]:.4f}\n")
        f.write(f"reward_yaw={reward[3]:.4f}\n")
    print(
        f"📝 Saved result.txt (reward_x={reward[0]:.4f},reward_y={reward[1]:.4f},reward_z={reward[2]:.4f},reward_yaw={reward[3]:.4f}\n)")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        current_time = loop_start - all_start
        real_time = total_loop_counter * timestep
        ...
        if loop_counter >= steps_between_pos_control:
            ...
            reward = reward_cal(reward, state, sim.targets[sim.current_target])
        ...
        # Exit when the max time reached
        if current_time >= max_allow:
            write_results(reward)
            p.disconnect()
            exit()
